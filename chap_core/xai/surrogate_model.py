...
SUPPORTED_MODELS is the single source of truth. Each entry defines:
  ...
  - shap_type: "tree" — determines which SHAP algorithm is used
...
SUPPORTED_MODELS: dict[str, dict[str, Any]] = {
    "gradient_boosting": {
        ...
        "default_params": {"n_estimators": 500, "max_depth": 6, "n_iter_no_change": 10, "validation_fraction": 0.15},
        "loo_params": {"n_estimators": 100, "max_depth": 4},
        # Spec format per param:
        #   {"type": "int"|"float", "low": ..., "high": ...|None,
        #    "log": bool (float only), "high_n_fraction": int (makes high = max(low, n//frac))}
        "tunable_params": {
    ...
    "random_forest": {
        ...
        "default_params": {"n_estimators": 1000, "max_depth": None},
        "loo_params": {"n_estimators": 60, "max_depth": 6},
        "tunable_params": {
    ...
    "hist_gradient_boosting": {
        ...
        "loo_params": {"max_iter": 100, "max_depth": 4},
        "tunable_params": {
...
def build_surrogate_model(
    ...
    if n_samples is not None and n_samples < 32:
        for key in ("n_iter_no_change", "validation_fraction", "early_stopping"):
            effective_params.pop(key, None)

    if n_samples is not None and n_samples < 30:
        for key, cap in (("max_depth", 4), ("max_leaf_nodes", 15)):
    ...
    # Ridge and some linear models don't accept random_state
    import inspect
...
def auto_select_best_model_type(
    ...
    return them ranked by LOO-R², best first.  Returns the top-2 types so
    callers can tune both and pick the winner.
...
def tune_surrogate_hyperparameters(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(splits):
            ...
            fold_score = -mean_squared_error(y[test_idx], m.predict(X[test_idx]))
    ...
    startup = max(2, min(5, n_trials // 2))
